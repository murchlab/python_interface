...
class Pulse:
    ...
    def copy(self):
        ...
        # (self, duration, start, amplitude, ssm_freq=None, phase=0, clock_freq=None, gaussian_bool=False, phase_ini=None, t_loop=None, ff=None):
...
#            return Pulse(self.duration, self.start, self.amplitude, gaussian_bool=self.gaussian_bool)
        return Pulse(self.duration, self.start, self.amplitude, ssm_freq=self.ssm_freq, phase=self.phase, clock_freq=self.clock_freq, gaussian_bool=self.gaussian_bool, phase_ini=self.phase_ini, t_loop=self.t_loop, ff=self.ff)
...
class Sequence:
    ...
    def load_sequence(self, instr_addr):
        ...
        print("loading")
        print("num_pats: {}".format(self.num_steps))
    ...
    def load_sequence_from_disk(self, instr_addr, base_name='foo', file_path=os.getcwd(), num_offset=0, amp12=1.5):
        ''' 
        DESCRIPTION: loads multi channel INPUT:
        OUTPUT:
        TODO:
