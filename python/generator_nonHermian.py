...
class Pulse:
    ...
    '''
    def __init__(self,duration, start, amplitude, ssm_freq=None, phase=0, gaussian_bool=False, phase_ini=None, t_loop=None, ff=None,detunlinear=0,detun=0,path_sign=1):
    ...
    def copy(self):
        ...
        if self.ssm_bool:
            return Pulse(self.duration, self.start, self.amplitude, self.ssm_freq, self.phase, self.gaussian_bool, self.phase_ini, self.t_loop, self.ff,self.detunlinear,self.detun,self.path_sign)
        else:
...
#        path_sign=self.path_sign
        ...
        if sweep_name == 'none':
            ...
            if not initial_pulse.ssm_bool: raise ValueError("Sweeping phase w/o SSM")
            for step_index, param_val in enumerate(np.linspace(start,stop,self.num_steps)):
                updated_pulse.phase= initial_pulse.phase + param_val*0.01*360
                gen_pulse( dest_wave = dest_wave[step_index], pulse=updated_pulse)
                
        elif sweep_name == 'detuned_phase':
            if not initial_pulse.ssm_bool: raise ValueError("Sweeping phase w/o SSM")
            for step_index, param_val in enumerate(np.linspace(start,stop,self.num_steps)):
                updated_pulse.phase= initial_pulse.phase+ (detun*t_loop*np.cos(2*np.pi*param_val/t_loop + phase_ini) - detun*t_loop)*path_sign*180/np.pi
                gen_pulse( dest_wave = dest_wave[step_index], pulse=updated_pulse)
...
def gen_pulse(dest_wave, pulse):
    ...
    if ff==None:
          ...
          if pulse.ssm_bool:
              ...
              phase_rad = phase/180.0*np.pi - 2*np.pi*detunlinear*(start)
