...
#                    binary_file += new_mark2
                    ...
                    new_mark2 = [ int(i*2**15) for i in mark2[step_index] ]
                    if write_binary:
                        binary_file = np.array([ round(2**14 *val + 8191.5) for val in channel[step_index] ])
                        binary_file = binary_file.clip(0, 2**14
                                                       -1)
                    else:
                        binary_file = np.array([ int(2**15 *val) for val in channel[step_index] ])
                    binary_file += new_mark1
                    binary_file += new_mark2
                    ###
