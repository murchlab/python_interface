...
def rabi_ge(num_patterns=51, sweep_time=200): # PMH 
...
#### all the below sequences do not have the correct loading syntax ####
...
def rabi_spectroscopy_probe_ge_sweep_ef(num_patterns=101, ssm_width=0.03): # PMH 
    sw_setup = setup_seq_writer()
    pat = get_pattern_parameters()
    expt_cal = get_expt_cal()
...
def ramsey_ef(num_patterns=101, sweep_time=500): # PMH 
    sw_setup = setup_seq_writer()
    pat = get_pattern_parameters()
    expt_cal = get_expt_cal()
...
def rabi_ge_prep_e(rabi_amp): # PMH 
    sw_setup = setup_seq_writer()
    pat = get_pattern_parameters()
    expt_cal = get_expt_cal()
...
#                                 stop=-sweep_time)
    ...
    p = gen.Pulse(start=end_time,
                  duration=0,
                  amplitude=rabi_amp, #expt_cal.pi_amp.ge,
                  ssm_freq=expt_cal.ssm.ge,
                  phase=0)
    seq = add_iq_pulse_sweep(seq=seq,
                                 pulse=p,
...
#    # do nothing
#    s = gen.Sequence(pat.pat_length, 1)
#    p = gen.Pulse(start=end_time,
#                  duration=-expt_cal.pi_time.ge,
#                  amplitude=0,
#                  ssm_freq=expt_cal.ssm.ge,
#                  phase=0)
#    s = add_iq_pulse_sweep(seq=s,
#                           pulse=p,
#                           sweep_name='none')
#    seq_list.append(s)
#    
#    # sweep amp for pi pulse
#    s = gen.Sequence(pat.pat_length, 3)
#    p = gen.Pulse(start=end_time,
#                  duration=-expt_cal.pi_time.ge,
#                  amplitude=0,
#                  ssm_freq=expt_cal.ssm.ge,
...
#    seq_list.append(s)
    ...
    p = gen.Pulse(start=end_time,
                  duration=-expt_cal.pi_time.ge,
                  amplitude=expt_cal.pi_amp.ge/2,
                  ssm_freq=expt_cal.ssm.ge,
                  phase=0)
    s = add_iq_pulse_sweep(seq=s,
                           pulse=p,
    ...
    pat.num_patterns = seq.num_steps
